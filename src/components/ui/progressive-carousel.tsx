...
export const ProgressSlider: FC<ProgressSliderProps> = ({
  ...
  useEffect(() => {
    ...
    if (getChildren && React.isValidElement(getChildren)) {
      const childProps = getChildren.props as { children?: ReactNode };
      const values = React.Children.toArray(childProps.children).map(
        (child) => {
      ...
      setSliderValues(values.filter(Boolean));
    }
  }, [children]);
